...
def game():
    ...
    if request.method == 'POST':
        ...
        game = Game(user_id=user_id, difficulty=difficulty, time_taken=time_taken, completed=completed, board=board_str, puzzle=json.dumps(submitted_board))
        db.session.add(game)
        db.session.commit()
    ...
    if request.headers.get('Accept') == 'application/json':
        print("BOARDDD:", board)# тут він бере board з початковими значеннями не даючи юзеру написати цифри
...
def stats():
    ...
    for game in games:
        ...
        # Get the solution board for the current game
        _, solution_board = sudoku_generator.generate_sudoku(difficulty_level)


        # Compare the submitted board with the solution board
        if game.puzzle == solution_board:
